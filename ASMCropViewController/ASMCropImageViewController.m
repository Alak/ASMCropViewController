...
- (void)setupImageView
{
	...
	self.cropView.zoomScale = 1;
    self.imageView.center = self.scrollView.center;
    self.cropView.imageFrame = self.imageView.frame;
}
...
{
    CGRect cropFrame = [self.cropView convertRect:self.cropView.cropFrame toView:self.imageView];
    
	CGImageRef subImage = CGImageCreateWithImageInRect(self.image.CGImage,
													   cropFrame);
	UIImage* croppedImage = [UIImage imageWithCGImage:subImage scale:1.0 orientation:self.image.imageOrientation];
